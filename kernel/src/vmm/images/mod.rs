...
fn load_images_mem(config: &AxVMCrateConfig) -> anyhow::Result<VMImagesConfig> {
    ...
    let bios = if let Some(bios) = memory_image.bios {
        Some(VMImageConfig {
    ...
    let dtb = if let Some(dtb) = memory_image.dtb {
        Some(VMImageConfig {
    ...
    let ramdisk = if let Some(ramdisk) = memory_image.ramdisk {
        Some(VMImageConfig {
...
pub mod fs {
    ...
    fn load_images_fs(config: &AxVMCrateConfig) -> anyhow::Result<VMImagesConfig> {
        todo!()
    }
}

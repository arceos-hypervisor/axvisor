...
fn gen_libos_configs() -> io::Result<()> {
    let mut output_file = open_output_file("shim_configs.rs");
    ...
    writeln!(
        output_file,
        "// Shim related configs, please refer to deps/shim for more info"
    )?;
    writeln!(output_file, "// Generated by build.rs, DO NOT edit!\n\n")?;

    writeln!(output_file, "/// Get shim kernel binary image.")?;
    ...
    writeln!(output_file, "/// Get gate process raw ELF data.")?;
    writeln!(
        output_file,
    ...
    // prefixing all constant definitions with SHIM_ and writing to output_file.
    let config_rs_path = "deps/shim/shim/src/platform/config.rs";
    let config_rs_content = fs::read_to_string(config_rs_path)
        .expect("Failed to read deps/shim/shim/src/platform/config.rs");

    for line in config_rs_content.lines() {
        // Only process lines with `pub const` definitions
        if let Some(rest) = line.strip_prefix("pub const ") {
            // Find the name and rest of the definition
            if let Some((name, rest)) = rest.split_once(':') {
                let name = name.trim();
                let rest = rest.trim_start();
                writeln!(output_file, "pub const SHIM_{}:{}", name, rest)?;
                continue;
            }
        }

        if line.starts_with("#!") {
            // Skip module comments, e.g., `#![allow(dead_code)]`
            continue;
        }

        // For other lines (attributes, comments, etc.), just write as is
